    ...
    try {
        ...
        for (const user of users) {
            if (user.profilePicture.includes('googleusercontent.com')) {
                // If it contains multiple =s or is clearly messed up, find the first =s and fix it
                let newPic = user.profilePicture;
                if (newPic.includes('=s')) {
                    const baseUrl = newPic.split('=s')[0];
                    newPic = baseUrl + '=s200-c';
                }

                if (newPic !== user.profilePicture) {
                    console.log(`Repairing ${user.displayName}: ${user.profilePicture} -> ${newPic}`);
